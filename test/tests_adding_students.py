...
class TestAddStudent(unittest.TestCase):
    ...
    @patch("builtins.input", side_effect=["", "Doe"]) # missing data (first name) - adding ivalid data :(
    def test_add_student_missing_first_name(self, mock_input):
        with self.assertRaises(ValueError):
            add_student()

    @patch("builtins.input", side_effect=["John", ""])  # missing data (last name) - adding ivalid data :((
