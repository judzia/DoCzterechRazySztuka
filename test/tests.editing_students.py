...
class TestEditStudent(unittest.TestCase):
    @patch(
        "builtins.input", side_effect=["John", "Doe", "Johnathan", "Doe"]
    )  # test - editing student
    @patch(
        "main.import_from_file",
        return_value=[{"first_name": "John", "last_name": "Doe", "present": False}],
    )
    ...
    def test_edit_student(self, mock_export, mock_import, mock_input):
        ...
        mock_export.assert_called_once_with(
            [{"first_name": "Johnathan", "last_name": "Doe", "present": False}]
